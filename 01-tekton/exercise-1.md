...
- 点击右上角的Login按钮，使用您自己的ibm account登录。
- 在右上角的下拉菜单中选择IBM。
- 点击右上角的命令行图标，根据提示输入passcode: ikslab
...
## 实验步骤
### 1. Fork tekton-tutorial 项目您你自己的repo并且clone到local workstation
...
4. 在您的本地目录`git clone https://github.com/<your git account>/tekton-tutorial.git`
...
- 登录UI https://cloud.ibm.com/login，切换到**您自己的**ibm account(很重要!不要使用IBM account)。
- 打开 https://cloud.ibm.com/iam/apikeys 页面， 点击“Create an IBM Cloud API key”按钮。
- 输入一个名字，点击"Create"按钮。
- Download API key，打开文件获取apikey。
- 使用刚刚得到的API key登录 ibmcloud。
...
5.2 下面我们来创建Tekton PipelineResource。名为picalc-git的PipelineResource指向一个git source。这git source是一个计算圆周率的go程序。它包含了一个Dockerfile来测试，编译代码，build image。    
```
apiVersion: tekton.dev/v1alpha1
...
- 检查taskruns的状态,直到的状态都是SUCCEEDED。
```
$ k get tr
...
```
- 如果看到以上结果，我们就可以查看部署好的Knative service了。READY状态为True说明部署成功了。    
```
...
kubectl get taskrun
```
- 如果有taskrun失败了，检查该taskrun的describe之后的‘Message’。   
```
